...
/*        if(data.province != "全国" || data.province != "广东"){
            $("#city").remove();
            $("#filter").append($("<div style=\"width: 750px;margin-top: 10px;float: left;\" id=\"city\"></div>"));
        }*/
        $("#content").append($("<div style=\"clear:both;\"></div>"));
        //设置隐藏当前页
        ...
        $("#selectCity li font").click(function () {
            ...
            if($("#select").html() == "全国"){
                ...
                $.ajax({
                    //请求地址
                    url:"http://localhost:8080/lostandfound/provinceController/getLostList2",
                    //提交方式
                    type:"POST",
                    //规定请求参数的数据类型为json格式，数据不能为对象
                    contentType:"application/json;charset=UTF-8",
    ...
    $.ajax({
        //请求地址
        url:"http://localhost:8080/lostandfound/provinceController/getLostList1",
        //提交方式
        type:"POST",
        //规定请求参数的数据类型为json格式，数据不能为对象
        contentType:"application/json;charset=UTF-8",
    ...
    $("#first").click(function () {
        ...
        if($("#select").html() == $("#shiWuProvince").html()){
            ...
            $.ajax({
                //请求地址
                url:"http://localhost:8080/lostandfound/provinceController/getLostList3",
                //提交方式
                type:"POST",
                //规定请求参数的数据类型为json格式，数据不能为对象
                contentType:"application/json;charset=UTF-8",
    ...
    $("#next").click(function () {
        ...
        if($("#select").html() == $("#shiWuProvince").html()){
            ...
            $.ajax({
                ...
                //规定请求参数的数据类型为json格式，数据不能为对象
                contentType:"application/json;charset=UTF-8",
                data:JSON.stringify({"currentPage":$("#hiddenPage").val()*1+1,"province":province,"city":city}),
                //响应的数据类型
                dataType:"JSON",
                success:function (data) {
                    page1(data);
    ...
    $("#selectType").change(function () {
        ...
        if($("#select").html() == $("#shiWuProvince").html()){
            ...
            var province = $("#select").html();
            $.ajax({
                url:"http://localhost:8080/lostandfound/searchController/search3",
                //提交方式
                type:"POST",
                //规定请求参数的数据类型为json格式，数据不能为对象
                contentType:"application/json;charset=UTF-8",
            ...
            var city = $("#shiWuProvince").html();
            $.ajax({
                url:"http://localhost:8080/lostandfound/searchController/search5",
                //提交方式
                type:"POST",
                //规定请求参数的数据类型为json格式，数据不能为对象
                contentType:"application/json;charset=UTF-8",
    ...
    $("#searchWord").click(function () {
        ...
        if($("#select").html() == $("#shiWuProvince").html()){
            ...
            if(type == "请选择"){
                ...
                $.ajax({
                    url:"http://localhost:8080/lostandfound/searchController/search7",
                    //提交方式
                    type:"POST",
                    //规定请求参数的数据类型为json格式，数据不能为对象
                    contentType:"application/json;charset=UTF-8",
            ...
            if(type == "请选择"){
                $.ajax({
                    url:"http://localhost:8080/lostandfound/searchController/search9",
                    //提交方式
                    type:"POST",
                    //规定请求参数的数据类型为json格式，数据不能为对象
                    contentType:"application/json;charset=UTF-8",
                ...
                $.ajax({
                    url:"http://localhost:8080/lostandfound/searchController/search11",
                    //提交方式
                    type:"POST",
                    //规定请求参数的数据类型为json格式，数据不能为对象
                    contentType:"application/json;charset=UTF-8",
